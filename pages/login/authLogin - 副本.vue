...
export default {
  ...
  methods: {
    ...
    clickLogin() {
      ...
      login(paramsObj)
        // this.$http
        //   .post(`/m/login?code=${code}&phone=${phoneNumbers}`)
        .then((e) => {
          // console.log('-------------',e);
          localStorage.setItem("amain_token", JSON.stringify(e.token));
          localStorage.setItem("loginPhone", JSON.stringify(phoneNumbers));
          if (e.code == 200) {
            Toast.loading({
              message: "登录成功",
              forbidClick: true,
    ...
    sendCode() {
      // this.$http.post("/m/common/login/api/sendCode?phoneNumbers=" + this.phoneNumbers).then(e => {
      this.$http.post("/m/sendCode?phone=" + this.phoneNumbers).then((e) => {
        if (e.code == 200) {
          this.$toast("发送成功");
        } else {
